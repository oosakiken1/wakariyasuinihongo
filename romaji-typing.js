...
function displayAll() {
    ...
    let displayText = hiraganaText + untransferText + missText;
    for (let i = 0; i < correctText.length; i++) {
        const span = document.getElementById('a' + i);
  
        if (i < hiraganaText.length) {
            if (correctText[i] === hiraganaText[i]) {
                span.classList.add('red');
                span.innerText = hiraganaText[i];
            } else {
                span.classList.remove('red');
                span.innerText = hiraganaText[i];
            } 
        } else if (i === hiraganaText.length) {
                span.classList.remove('red');
                span.innerHTML = `<span class="main">${untransferText}${missText}</span><span class="cursor">|</span>`;
        } else {
            span.classList.remove('red');
            span.innerText = '';
            // span.innerText = correctText[i];
        }
    }
                ...
                for (let char of str) {
                    if (char !== '　') {
                        if (config.ruby === 'r0') {  // ふりがなすべての時は、見える
                            ansText = `<span class="ans" id="a${rbIndex}">${char}</span>`;
                            rbText += `<td>${ansText}<span class="main" id="r${rbIndex}">${char}</span></td>`;
                        } else {                    // ふりがなすべて以外は、見えない
                            ansText = `<span class="ans" id="a${rbIndex}">${char}</span>`;
                            rbText += `<td>${ansText}<span class="main white" id="r${rbIndex}">${char}</span></td>`;
                        }
                        rbIndex++;
                    } else {
                        ansText = `<span></span>`;
                        rbText += `<td><span></span>${ansText}</td>`;
                    }
                }
            //ふりがなブロックの場合
            } else {
                ...
                //ふりがなテキストに追加
                rbText += '<td class="narrow">'
                for (let char of str) {
                    if (config.ruby !== 'r2') {  // ふりがななし以外は、見える
                        ansText = `<span class="ans" id="a${rbIndex}">${char}</span>`;
                        rbText += `${ansText}<span class="main" id="r${rbIndex}">${char}</span>`;
                    } else {                    // ふりがななしの時は、見えない
                        ansText = `<span class="ans" id="a${rbIndex}">${char}</span>`;
                        rbText += `${ansText}<span class="main white" id="r${rbIndex}">${char}</span>`;
                    }
                    rbIndex++;
                }
                rbText += '</th>'
...
BTN_START.onclick = function () {
    ...
    gtag('event', 'start-romaji-typing', {'event_category': 'execute', 'config': tmpConfig});
