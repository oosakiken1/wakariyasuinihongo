...
BTN_START.onclick = function () {
    ...
    let tmpConfig = `ji-t:${config.time}-l:${config.level}-o:${config.order}-a:${config.audio}-p:${config.pass}`
    gtag('event', 'start-japanese-input', {'event_category': 'execute', 'config': tmpConfig});
    // gtag('event', 'start-japanese-input', {'event_category': 'execute', 'config': JSON.stringify(config)});
